...
\node [rule, fill=lccyan!25] (P)			{Présentation\\ (navigateur Web)};
% Use join to connect a node to the previous one 
\node [rule, fill=lccyan!25, join=by double] (S)	{Métier\\ (Serveur Web)};
\node [rule, fill=lccyan!25, join]	(DB)		{Données\\ (Serveur SQL)};
